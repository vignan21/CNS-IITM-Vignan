...
% Monophasic waveform with different configurations
...
if scp==1
    tempdbs=monophasic(amp,dc,Ttime,freq);
end

if fcp==1
    fcp=4;
    tempdbs=zeros(fcp,Ttime+1);
    temp=monophasic_random(amp,dc,Ttime+2000,freq);
    for i=1:fcp
...
for k = 1:Ttime
    ...
    %------------------------------DBS current four point--------------------------------%
        if fcp==1
            radiusdbs=2;
            Idbs=fourpointdbs(Idbs,tempdbs,radiusdbs,k);
        end
